...
const int *PackW64PeArm64::getCompressionMethods(int method, int level) const {
    ...
    static const int m_lz4[] = {M_LZ4, M_END};
    static const int m_misa77[] = {M_MISA77, M_END};
    if (M_IS_LZ4(method))
        return m_lz4;
    if (M_IS_MISA77(method))
        return m_misa77;
...
void PackW64PeArm64::buildLoader(const Filter *ft) {
    ...
    if (opt->nextgen.release_memory && (isdll || isefi))
        throwCantPack("--release-memory for win64/arm64 is supported only for executable images");

    initLoader(EM_AARCH64, stub_arm64_win64_pe, sizeof(stub_arm64_win64_pe), 2);

    addLoader("START", "PEMAIN01", "PEMAIN02");
    addLoader(M_IS_LZ4(ph.method)     ? "PECALLL4"
              : M_IS_MISA77(ph.method) ? "PECALLM7"
    ...
    if (use_tls_callbacks)
        addLoader("PETLSC");
    addLoader("PEDOJUMP");
    addLoader(M_IS_LZ4(ph.method) ? "LZ4"
              : M_IS_MISA77(ph.method) ? "MISA77"
                                       : "NRV_HEAD");
    if (!M_IS_LZ4(ph.method) && !M_IS_MISA77(ph.method))
