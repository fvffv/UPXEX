...
static noinline void check_and_update_options(int i, int argc) {
    ...
    check_not_both(opt->nextgen.lz4, opt->nextgen.lz4_hc, "--lz4", "--lz4-hc");
    check_not_both(opt->nextgen.lz4_hc, opt->nextgen.lz4_acceleration_set, "--lz4-hc",
                   "--lz4-acceleration");
...
static noinline int do_option(int optc, const char *arg) {
    ...
    case 724:
        ...
        break;
    case 725: // --lz4
        opt->nextgen.lz4 = true;
        opt->nextgen.misa77 = false;
        if (!set_method(M_LZ4, 1))
            e_method(M_LZ4, 1);
        break;
    case 726: // --lz4-hc
        opt->nextgen.lz4_hc = true;
        opt->nextgen.misa77 = false;
        if (!set_method(M_LZ4, 9))
            e_method(M_LZ4, 9);
        break;
    case 727: // --lz4-acceleration=N
        getoptvar(&opt->nextgen.lz4_acceleration, 1, 65537, arg);
        opt->nextgen.lz4_acceleration_set = true;
        break;
    case 728: // --misa77
        opt->nextgen.misa77 = true;
        opt->nextgen.lz4 = false;
        opt->nextgen.lz4_hc = false;
        opt->nextgen.misa77_level = 1;
        if (!set_method(M_MISA77, 3))
            e_method(M_MISA77, 3);
        break;
    case 729: { // --misa77-level=-1..4
        int misa_level = 1;
        getoptvar(&misa_level, -1, 4, arg);
        opt->nextgen.misa77 = true;
        opt->nextgen.lz4 = false;
        opt->nextgen.lz4_hc = false;
        opt->nextgen.misa77_level = misa_level;
        if (!set_method(M_MISA77, misa_level + 2))
            e_method(M_MISA77, misa_level + 2);
        break;
    }

    // Next Generation execution profiles
    case 912: // --fast-start
        opt->nextgen.fast_start = true;
        opt->nextgen.release_memory = true;
        opt->o_unix.unmap_all_pages = true;
        opt->nextgen.lz4 = false;
        opt->nextgen.lz4_hc = false;
        opt->nextgen.misa77 = true;
        opt->nextgen.misa77_level = 1;
        if (!set_method(M_MISA77, 3))
            e_method(M_MISA77, 3);
        break;
    case 913: // --release-memory
        opt->nextgen.release_memory = true;
        opt->o_unix.unmap_all_pages = true;
        break;
    case 914: // --stub-timing
        opt->nextgen.stub_timing = true;
        break;
    // compression level
    case '1':
    case '2':
...
int main_get_options(int argc, char **argv) {
    ...
    static const struct mfx_option longopts[] = {
        ...
        {"analyze", 0x10, N, 911},     // Next Generation executable analysis
        ...
        {"lz4", 0x10, N, 725},      // --lz4 fast provider
        {"lz4-hc", 0x10, N, 726},   // --lz4 high-compression provider
        {"lz4-acceleration", 0x31, N, 727}, // --lz4-acceleration=
        {"misa77", 0x10, N, 728},           // --misa77 (default level 1)
        {"misa77-level", 0x31, N, 729},     // --misa77-level=-1..4
        ...
        {"small", 0x10, N, 520},
        {"fast-start", 0x10, N, 912},
        {"release-memory", 0x10, N, 913},
        {"stub-timing", 0x10, N, 914},
        // CRP - Compression Runtime Parameters (undocumented and subject to change)
...
#endif
#if (WITH_LZ4)
    assert(upx_lz4_init() == 0);
#endif
#if (WITH_MISA77)
    assert(upx_misa77_init() == 0);
#endif
