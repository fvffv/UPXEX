...
void PackW64PeAmd64::addStubImports() {
    super::addStubImports();
    if (opt->nextgen.release_memory) {
        addKernelImport("CreateThread");
        addKernelImport("CloseHandle");
        addKernelImport("Sleep");
        addKernelImport("SetProcessWorkingSetSize");
    }
    if (opt->nextgen.stub_timing) {
        addKernelImport("QueryPerformanceCounter");
        addKernelImport("QueryPerformanceFrequency");
        addKernelImport("CreateFileA");
        addKernelImport("WriteFile");
        if (!opt->nextgen.release_memory)
            addKernelImport("CloseHandle");
    }
}
...
void PackW64PeAmd64::buildLoader(const Filter *ft) {
    if (opt->nextgen.release_memory && (isdll || isefi))
        throwCantPack("--release-memory for win64/pe is supported only for executable images");
    if (opt->nextgen.stub_timing && (isdll || isefi))
        throwCantPack("--stub-timing for win64/pe is supported only for executable images");
    ...
    addLoader(isdll ? "PEISDLL1" : "", "PEMAIN01", // outer "enter" (push rbp; mov rsp,rbp)
              ...
              opt->nextgen.stub_timing ? "PETIMING0" : "",
              ...
              : M_IS_LZ4(ph.method) ? "LZ4"

              : M_IS_MISA77(ph.method) ? "MISA77"
    ...
    if (opt->nextgen.stub_timing)
        addLoader("PETIMING4,PETIMING_WRITE");
    ...
    addLoader(opt->nextgen.stub_timing ? "PETIMING_DATA" : "", "IDENTSTR,UPX1HEAD");
}
...
void PackW64PeAmd64::defineSymbols(unsigned ncsection, unsigned upxsection, unsigned sizeof_oh,
    ...
    if (ilinker) {
        ...
        if (opt->nextgen.release_memory) {
            linker->defineSymbol("CreateThread",
                                 ilinkerGetAddress("kernel32.dll", "CreateThread"));
            linker->defineSymbol("CloseHandle",
                                 ilinkerGetAddress("kernel32.dll", "CloseHandle"));
            linker->defineSymbol("Sleep", ilinkerGetAddress("kernel32.dll", "Sleep"));
            linker->defineSymbol(
                "SetProcessWorkingSetSize",
                ilinkerGetAddress("kernel32.dll", "SetProcessWorkingSetSize"));
        }
        if (opt->nextgen.stub_timing) {
            linker->defineSymbol(
                "QueryPerformanceCounter",
                ilinkerGetAddress("kernel32.dll", "QueryPerformanceCounter"));
            linker->defineSymbol(
                "QueryPerformanceFrequency",
                ilinkerGetAddress("kernel32.dll", "QueryPerformanceFrequency"));
            linker->defineSymbol("CreateFileA", ilinkerGetAddress("kernel32.dll", "CreateFileA"));
            linker->defineSymbol("WriteFile", ilinkerGetAddress("kernel32.dll", "WriteFile"));
            linker->defineSymbol("TimingCloseHandle",
                                 ilinkerGetAddress("kernel32.dll", "CloseHandle"));
        }
    }
    ...
    if (M_IS_LZ4(ph.method)) {
        linker->defineSymbol("lz4_c_len", ph.c_len);
        linker->defineSymbol("lz4_u_len", ph.u_len);
    }
    if (M_IS_MISA77(ph.method)) {
        linker->defineSymbol("misa77_c_len", ph.c_len);
        linker->defineSymbol("misa77_u_len", ph.u_len);
    }
