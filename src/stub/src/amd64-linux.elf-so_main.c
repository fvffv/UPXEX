...
{
    // Keep these instructions as scalar immediates.  Modern AArch64 LLVM
    // otherwise pools the 16 bytes in .rodata.cst16, which the folded loader
    // cannot carry as a separately relocated section.
    volatile unsigned code[4];
    code[0] = 0xd4000001;  // svc #0
    code[1] = 0xa9417be2;  // ldp x2,lr,[sp,#2*8)]
    code[2] = 0xa8c207e0;  // ldp x0,x1,[sp], 4*8
    code[3] = 0xd61f03c0;  // br x30
    unsigned *hatch = 0;
    ...
    if (phdr->p_type==PT_LOAD && phdr->p_flags & PF_X) {
        ...
        if (4*4 <= frag_mask) {
            ...
            int mfd = memfd_create(addr_string("upx"), 0);  // the directory entry
            Pwrite(mfd, (void const *)code, sizeof(code));
