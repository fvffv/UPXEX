...
void Packer::doPack(OutputFile *fo) {
    if (opt->nextgen.stub_timing && getFormat() != UPX_F_W64PE_AMD64)
        throwCantPack("--stub-timing is currently supported only for win64/pe x86-64 executables");
    if ((opt->nextgen.fast_start || opt->nextgen.release_memory || opt->nextgen.lz4 ||
         opt->nextgen.lz4_hc || opt->nextgen.misa77) &&
        !upx::nextgen::supportsNextGenerationTarget(getFormat()))
        throwCantPack("UPX Next Generation profile is not supported for this executable format");
    if (opt->nextgen.release_memory && !upx::nextgen::supportsReleaseMemory(getFormat()))
        throwCantPack("--release-memory is not safely supported by this target loader");
    if ((opt->nextgen.lz4 || opt->nextgen.lz4_hc || M_IS_LZ4(opt->method)) &&
        !upx::nextgen::hasLz4Runtime(getFormat()))
        throwCantPack("LZ4 host provider is available, but this target stub has no LZ4 runtime decoder");
    if ((opt->nextgen.misa77 || M_IS_MISA77(opt->method)) &&
        !upx::nextgen::hasMisa77Runtime(getFormat()))
        throwCantPack("misa77 is currently supported only by the win64/pe x86-64 runtime");
    uip->uiPackStart(fo);
    pack(fo);
    uip->uiPackEnd(fo);
}
...
bool Packer::compress(SPAN_P(byte) i_ptr, unsigned i_len, SPAN_P(byte) o_ptr,
    ...
    // Some one-shot compressors (notably LZ4 and misa77) cannot report
    // intermediate input positions.  Still publish an accurate completion
    // event so their progress display transitions from 0% to 100%.
    if (r == UPX_E_OK)
