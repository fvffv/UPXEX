...
void show_help(int verbose) {
    ...
    if (verbose > 0)
    {
        ...
        fg = con_fg(f, fg);
        con_fprintf(f,
                    "  --fast-start        select misa77 level 1 and reclaim startup pages\n"
                    "  --release-memory    reclaim decompression working-set pages after startup\n"
                    "  --stub-timing       write Win64/x64 stub phase timings to upx-stub-timing.bin\n"
                    "  --analyze           report size and runtime measurement status\n"
                    "  --lz4               select the LZ4 fast provider\n"
                    "  --lz4-acceleration=N tune LZ4 fast mode [1..65537, default 16]\n"
                    "  --lz4-hc            select the LZ4 high-compression provider\n"
                    "  --misa77            select misa77 with decompression-first level 1\n"
                    "  --misa77-level=N    select misa77 level [-1..4, default 1]\n"
...
#endif
    v = upx::nextgen::upx_lz4_version_string();
    if (v != nullptr && v[0])
        fprintf(f, "LZ4 data compression library %s\n", v);
#if (WITH_MISA77)
    v = upx::nextgen::upx_misa77_version_string();
    if (v != nullptr && v[0])
        fprintf(f, "misa77 data compression library %s\n", v);
#endif
#if (WITH_ZSTD)
