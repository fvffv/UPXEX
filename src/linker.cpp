...
#endif
        {
            // GNU and LLVM emit local ARM/AArch64 mapping symbols such as
            // $x, $d, $a and $t in every code/data section.  They describe
            // disassembly state, are never relocation targets, and their
            // names are intentionally repeated across sections.  The runtime
            // stub linker requires unique symbol names, so do not import them.
            if (symbol[0] == '$' && strchr("adtx", symbol[1]) != nullptr &&
                (symbol[2] == '\0' || symbol[2] == '.'))
                continue;
            char *s = strstr(start, symbol);
            s[strlen(symbol)] = 0;
...
void ElfLinker::defineSymbol(const char *name, upx_uint64_t value) {
    Symbol *symbol = findSymbol(name, false);
    // LLVM MC can omit the symbol-table entry for an empty named section
    // (notably the Win64 START anchor). Treat that section name exactly like
    // the section symbol emitted by the historical GNU assembler.
    if (symbol == nullptr) {
        Section *section = findSection(name, false);
        if (section != nullptr) {
...
upx_uint64_t ElfLinker::getSymbolOffset(const char *name) const {
    const Symbol *symbol = findSymbol(name, false);
    // Modern assemblers may omit a symbol-table entry for a named section.
    // Its linked section offset is the same anchor used by the historical
    // GNU section symbol, so accept it here as defineSymbol() already does.
    if (symbol == nullptr) {
        const Section *section = findSection(name, false);
        if (section == nullptr)
            throwInternalError("unknown symbol %s\n", name);
        if (section->output == nullptr)
            return 0xdeaddead;
