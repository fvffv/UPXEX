...
def do_file(fn):
    ...
    if opts.with_dump:
        ...
        if pos < 0:
            # LLVM objcopy may order .shstrtab before .strtab, so the old
            # contiguous marker is absent.  strip_with_dump has already
            # found the exact end of every CONTENTS section.
            assert len(odata) <= len(idata)
        elif re.search(r"^powerpc64", os.path.basename(fn)):
            ...
            # Modern GNU ld, like modern LLVM objcopy, may leave non-CONTENTS
            # metadata between the last loadable section and the string
            # tables.  strip_with_dump() computed the exact CONTENTS boundary,
            # so only reject layouts where a string table overlaps that data.
            assert pos >= len(odata), ("unexpected strip_with_dump", pos, len(odata))
    else:
        if pos >= 0:
            odata = idata[:pos]
    ...
    if eh and odata and not opts.dry_run:
        ...
        fp.truncate()
    fp.close()
    if opts.append_dump and opts.with_dump and not opts.dry_run:
        with open(fn, "ab") as output, open(opts.with_dump, "rb") as dump:
...
def main(argv):
    ...
    shortopts, longopts = "qv", [
        "append-dump", "dry-run", "quiet", "verbose", "with-dump="
    ]
    ...
    for opt, optarg in xopts:
        ...
        elif opt in ["--append-dump"]: opts.append_dump = opts.append_dump + 1
