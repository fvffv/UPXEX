...
void UiPacker::doCallback(unsigned isize, unsigned osize) {
    ...
    *m++ = progress_filler[3];
    ...
    // This value is displayed next to the progress bar, so report completion
    // progress rather than the transient compression ratio.  The old fallback
    // was 100.0% for the initial (0, 0) callback, which made one-shot
    // compressors such as LZ4 and misa77 look permanently complete.
    const unsigned progress = get_ratio(s->u_len, isize);

    int buflen = (int) (&s->msg_buf[sizeof(s->msg_buf)] - m);
    upx_safe_snprintf(m, buflen, "  %3d.%1d%%  %c ", progress / 10000,
                      (progress % 10000) / 1000, spinner[s->spin_counter & 3]);
...
bool UiPacker::uiFileInfoStart() {
    ...
    if (pb->ph.c_len > 0) {
        ...
        if (opt->nextgen.analyze) {
            con_fprintf(stdout, "  Original Size:    %u bytes\n", pb->ph.u_file_size);
            con_fprintf(stdout, "  Compressed Size:  %llu bytes\n", pb->file_size_u);
            con_fprintf(stdout, "  Startup Overhead: not measured (run the benchmark harness)\n");
            con_fprintf(stdout, "  Runtime Memory:   not measured (run the benchmark harness)\n");
        }
        return false;
    } else {
        con_fprintf(stdout, "  %8llu bytes", pb->file_size_u);
        ...
        if (opt->nextgen.analyze) {
            con_fprintf(stdout, "  Original Size:    %llu bytes\n", pb->file_size_u);
            con_fprintf(stdout, "  Compressed Size:  %llu bytes\n", pb->file_size_u);
            con_fprintf(stdout, "  Startup Overhead: 0 ms (not compressed)\n");
            con_fprintf(stdout, "  Runtime Memory:   baseline executable (not measured)\n");
        }
        return true;
    }
}
