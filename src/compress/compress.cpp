...
#endif
#if (WITH_LZ4)
    else if (M_IS_LZ4(method))
        r = upx_lz4_compress(src, src_len, dst, dst_len, cb, method, level, cconf, cresult);
#endif
#if (WITH_MISA77)
    else if (M_IS_MISA77(method))
        r = upx_misa77_compress(src, src_len, dst, dst_len, cb, method, level, cconf, cresult);
#endif
#if (WITH_NRV)
...
int upx_decompress(const upx_bytep src, unsigned src_len, upx_bytep dst, unsigned *dst_len,
    ...
    if (__acc_cte(false)) {
        ...
        r = upx_lz4_decompress(src, src_len, dst, dst_len, method, cresult);
#endif
#if (WITH_MISA77)
    else if (M_IS_MISA77(method))
        r = upx_misa77_decompress(src, src_len, dst, dst_len, method, cresult);
#endif
#if (WITH_NRV)
    else if ((M_IS_NRV2B(method) || M_IS_NRV2D(method) || M_IS_NRV2E(method)) && !opt->prefer_ucl)
...
int upx_test_overlap(const upx_bytep buf, const upx_bytep tbuf, unsigned src_off, unsigned src_len,
    ...
    if (__acc_cte(false)) {
        ...
        r = upx_lz4_test_overlap(buf, tbuf, src_off, src_len, dst_len, method, cresult);
#endif
#if (WITH_MISA77)
    else if (M_IS_MISA77(method))
        r = upx_misa77_test_overlap(buf, tbuf, src_off, src_len, dst_len, method, cresult);
#endif
#if (WITH_NRV)
    else if ((M_IS_NRV2B(method) || M_IS_NRV2D(method) || M_IS_NRV2E(method)) && !opt->prefer_ucl)
