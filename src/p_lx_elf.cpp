...
{
    // The normal ARM32 ELF exit path already unmaps both the compressed
    // C_TEXT mapping and the unfolded stub.  The older unmap_all_pages path
    // additionally rewrites the kernel-provided initial stack in place; on
    // ARM32 PIE executables that can corrupt argv/envp/auxv before control is
    // transferred to ld-linux.  Keep ARM32 on the normal reclaim path while
    // retaining the aggressive mode for targets where it is safe.
    const bool unmap_all_pages = opt->o_unix.unmap_all_pages &&
...
//        b_len + ph.c_len );
    ...
    addLoader(M_IS_MISA77(ph.method) ? "MISA77"
              : M_IS_LZ4(ph.method) ? "LZ4"
                                    : getDecompressorSections(), nullptr);
...
//   SO_MAIN  C-language supervision based on PT_LOADs
        ...
        if ((1u<<M_MISA77) & m_decompr) {
            len += snprintf(&sec[len], sizeof(sec) - len, ",%s", "MISA77");
        }
        if ((1u<<M_LZ4) & m_decompr) {
            len += snprintf(&sec[len], sizeof(sec) - len, ",%s", "LZ4");
        }
...
// and see if appending ARM_ATTRIBUTES Shdr is good enough.
    ...
    if (Elf32_Ehdr::ET_DYN==get_te16(&ehdr->e_type)) {
        ...
        // Shared objects use a DT_INIT trampoline while the dynamic linker is
        // still relocating the image.  Their compressed mapping cannot be
        // discarded with the executable-image release path.
        if (opt->nextgen.release_memory) {
            throwCantPack("--release-memory for Linux ELF is supported only for executable images");
        }
