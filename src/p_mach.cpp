...
void PackMachAMD64::addStubEntrySections(Filter const * /*ft*/)
{
   ...
   //addLoader(getDecompressorSections(), nullptr);
    addLoader(
        ...
        : M_IS_NRV2D(ph.method) ? "NRV_HEAD,NRV2D,NRV_TAIL"
        : M_IS_NRV2B(ph.method) ? "NRV_HEAD,NRV2B,NRV_TAIL"
        : M_IS_LZ4(ph.method)  ? "NRV_HEAD,LZ4,NRV_TAIL"
        : M_IS_MISA77(ph.method) ? "NRV_HEAD,MISA77,NRV_TAIL"
        : M_IS_LZMA(ph.method)  ? "LZMA_ELF00,LZMA_DEC20,LZMA_DEC30"
        : nullptr), nullptr);
    if (hasLoaderSection("CFLUSH"))
        addLoader("CFLUSH");
...
{
    // FOLDEXEC is synthesized below and therefore is not part of the embedded
    // entry-stub size used by ElfLinker::init() for its output capacity.  Fast
    // decoders can compress the folded loader less densely than NRV, so reserve
    // the full uncompressed fold size plus room for b_info and IDENTSTR.
    initLoader(0, proto, szproto, -1, szfold + 256);
  ...
  if (0 < szfold) {
    ...
    MemBuffer cprLoader_buf;
    cprLoader_buf.allocForCompression(sizeof(h) + h.sz_unc);
    unsigned char *const cprLoader = (unsigned char *)cprLoader_buf.getVoidPtr();
  if (0 < szfold) {
    unsigned sz_cpr = cprLoader_buf.getSize() - sizeof(h);
    // misa77 level 4 is optimized for large payloads and can expand this tiny
    // folded loader.  Its decoder accepts both stream formats, so use the
    // level-1 light stream for the loader while preserving the user's selected
    // level for the executable payload itself.
    const int loader_level = M_IS_MISA77(ph.method) ? 3 : 10;
    int r = upx_compress(uncLoader, h.sz_unc, sizeof(h) + cprLoader, &sz_cpr,
        nullptr, ph.method, loader_level, nullptr, nullptr );
    h.sz_cpr = sz_cpr;
...
int  PackMachBase<T>::pack2(OutputFile *fo, Filter &ft)  // append compressed body
{
    ...
    &&  0!=msegcmd[k].filesize ) {
        ...
        // The Mach runtime maps compressed input from the packed file and
        // output segments into distinct VM regions (do_xmap/unpackExtent), so
        // the generic Unix in-place-overlap test does not describe this path.
        packExtent(x,
            (do_filter ? &ft : nullptr), fo, hdr_u_len, b_extra, true );
        if (do_filter) {
            exe_filesize_max = 0;
        }
        hdr_u_len = 0;
...
tribool PackMachBase<T>::canUnpack()
{
    ...
    if (391 == style) { // PackHeader precedes __LINKEDIT
        if ((off_t)bufsize > offLINK)
            bufsize = (unsigned)offLINK;
        if (bufsize < sizeof(struct b_info))
            throwCantUnpack("bad offLINK %p %p", (void *)offLINK, (void *)file_size);
