...
void PackW32PeI386::buildLoader(const Filter *ft) {
    if (opt->nextgen.release_memory && (isdll || isefi))
        throwCantPack("--release-memory for win32/pe is supported only for executable images");
...
void PackW32PeI386::addStubImports() {
...
void PackW32PeI386::defineSymbols(unsigned ncsection, unsigned upxsection, unsigned sizeof_oh,
    ...
    if (ilinker) {
        ...
        if (opt->nextgen.release_memory) {
            linker->defineSymbol(
                "CreateThread",
                0u - rvamin + ilinkerGetAddress("kernel32.dll", "CreateThread"));
            linker->defineSymbol(
                "CloseHandle",
                0u - rvamin + ilinkerGetAddress("kernel32.dll", "CloseHandle"));
            linker->defineSymbol("Sleep",
                                 0u - rvamin + ilinkerGetAddress("kernel32.dll", "Sleep"));
            linker->defineSymbol(
    ...
    if (M_IS_LZ4(ph.method)) {
        // The i386 decoder stops at end-of-input and only relocates the compressed length.
